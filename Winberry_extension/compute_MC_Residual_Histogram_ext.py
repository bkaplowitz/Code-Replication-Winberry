...
def compute_MC_Residual_Histogram(
    ...
    residual = capital - np.sum(
        np.multiply(
            assets_grid_fine.ravel(order="F"),
...
def update_coefs_poly(
    ...
    assets_prime_grid = np.tile(
        assets_prime.reshape((1, n_states), order="F"), (n_epsilon, 1)
    )
