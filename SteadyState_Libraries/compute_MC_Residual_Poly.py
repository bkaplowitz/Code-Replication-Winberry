    ...
    while (err > err_2) & (iter <= tol_2):
        for i_epsilon in np.arange(0, n_epsilon):
            ...
            normalization = exp_func_error(
                params_temp[i_epsilon, :],
                grid_moments[i_epsilon, :, :].squeeze(),
                quad_weights,
                n_measure,
            ).reshape((1,), order="F")
            # make sure works.
            params_new[i_epsilon, :] = np.hstack(
                (1 / normalization, params_temp[i_epsilon, :])
            )
        ...
        moments_new, grid_moments_new = compute_moments(
            grid_moments,
            constrained,
            epsilon_trans_mat,
            n_epsilon,
        ...
        # compute returns
        capital_new = compute_capital(
            moments, constrained, n_epsilon, epsilon_invariant, a_bar
        )
...
def assets_prime_grids(epsilon_grid, epsilon_grid_quad, n_epsilon, n_assets_quad, assets_min, assets_poly_BC, assets_poly_quad, assets_grid_quad, a_bar, mu, sigma, tau, r, w, coefs):
    ...
    # compute policies at borrowing constraint:,    # compute cond expec approx at borrowing constraint
    ...
    return assets_prime_quad,assets_prime_BC
...
def compute_coefs(
    ...
    # fit/find coefs using chebyshev interpolation trick, simplified form of projection,    # also uses trick from i:i+1 to return 1xn vec
...
@njit
def compute_capital(moments, constrained, n_epsilon, epsilon_invariant, a_bar):
    '''
...
def compute_moments(
    ...
    for i_epsilon_prime in np.arange(0, n_epsilon):
        ...
        for i_epsilon in np.arange(0, n_epsilon):
            grid_moments_slice = grid_moments[i_epsilon, :, :].copy()
            ...
            moments_new[i_epsilon_prime, 0] = (
                ...
                * (
                    ...
                    @ (
                        np.multiply(
                            assets_prime_quad[i_epsilon, :],
                            np.exp(
                                (
                                    grid_moments_slice.reshape((n_assets_quad,n_measure))
                                    @ params_new[i_epsilon, 1 : n_measure + 1]
                                )
                            )
                        )
                    )
    ...
    return [moments_new, grid_moments_new]
...
def exp_func_error(params, grid_moments, quad_weights, n_measure):
    ...
    """
    params_vec = np.reshape(np.asarray(params),(n_measure, 1))
...
def deriv_exp_func(params, grid_moments, quad_weights, n_measure):
    ...
    # 0 on last line tells sum to sum over axis 0. From FOC
    deriv = np.sum(
        np.multiply(
